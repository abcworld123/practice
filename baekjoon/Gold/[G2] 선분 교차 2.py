...
if check == '--':
    if y1 == y3 and x3 <= x2: p(1)

elif check == '||':
    if x1 == x3 and y3 <= y2: p(1)

elif check == '-|':
    if x1 <= x3 <= x2 and y3 <= y1 <= y4: p(1)
    ...
    if check == '-/':
        x = Fraction(y1 - d, c)
        if min(y3, y4) <= y1 <= max(y3, y4) and x1 <= x <= x2: p(1)

    elif check == '|/':
        y = c * x1 + d
        if x3 <= x1 <= x4 and y1 <= y <= y2: p(1)

    else:  # '//', '/\'
        a = Fraction(y2 - y1, x2 - x1)
        b = y1 - x1 * a
        if a == c:
            if b == d and x3 <= x2: p(1)
        else:
            x = Fraction(d - b, a - c)
            if x1 <= x <= x2 and x3 <= x <= x4: p(1)
